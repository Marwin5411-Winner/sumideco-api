...
exports.createShop = async (req, res) => {
    ...
    try {
      ...
      if (!tarvation_user) {
        tarvation_user = await mongoose.ShopCustomers.create({ name, email, password: hashedPassword });
      }

        const paxy_shop = await sequelize.shops.findOne({ where: { email } });
    
        if (paxy_shop) {
            return res.status(400).send(global.HTTP_CODE.BAD_REQUEST + ": Shop already exists");
            }
      ...
      await sequelize.shops
        .create({ ssoId: tarvation_user._id.toString(), name, shop_title, shop_description})
        .then((e) => {
